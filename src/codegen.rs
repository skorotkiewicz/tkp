...
impl CodeGen {
    ...
    fn gen_checked_index_load(&mut self, object: &Expr, index: &Expr) -> String {
        ...
        self.emit(&format!("{}:", error_lbl));
        self.record_runtime_error("Index out of bounds");
        self.emit(&format!("  br label %{}", end_lbl));
        ...
        match expr {
            ...
            Expr::Lambda { params, body, .. } => {
                ...
                let typed_params: Vec<(String, crate::ast::Type)> = params
                    .iter()
                    .map(|(name, ty)| (name.clone(), ty.clone().unwrap_or(crate::ast::Type::Nanpa)))
                    .collect();
                self.gen_func(
                    &lambda_name,
                    &typed_params,
                    &Some(crate::ast::Type::Nanpa),
                    body,
                );
    ...
    fn guess_struct_type(&self, expr: &Expr) -> String {
        if let Expr::Identifier(name) = expr {
            if let Some(struct_name) = self.var_types.get(name.as_str()) {
                // This assumes var_types stores the actual struct name if it's a struct type.
                // If it stores LLVM types like "i64*", this logic needs refinement.
                // For now, we'll return the stored type if it exists.
    ...
    pub fn generate(&mut self, program: &Program) -> String {
        ...
        module.push_str("; TKP Language Generated IR\n");
...
mod tests {
    ...
    fn test_codegen_hello() {
        let program = Program::new(vec![make_print_call("toki a!")]);
    ...
    fn test_codegen_function() {
        let program = Program::new(vec![Stmt::unspanned(StmtKind::FuncDef {
            name: "add".to_string(),
            params: vec![
                ("a".to_string(), Type::Nanpa),
                ("b".to_string(), Type::Nanpa),
            ],
    ...
    fn test_codegen_module_header() {
        ...
        let ir = codegen(&program);
        assert!(ir.contains("; TKP Language Generated IR"));
